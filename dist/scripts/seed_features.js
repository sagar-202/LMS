...
async function seed() {
    ...
    const [videos] = await c.query('SELECT id, title FROM videos LIMIT 5');
    console.log('Found videos:', videos.map((v) => `[${v.id}] ${v.title}`).join(', '));
    if (!videos.length) {
    ...
    const subjectId = 27;
    const [enrollments] = await c.query('SELECT * FROM enrollments WHERE user_id = ? AND subject_id = ?', [user.id, subjectId]);
    ...
    const [existing] = await c.query('SELECT id FROM quizzes WHERE video_id = ?', [targetVideoId]);
    let quizId;
    if (existing.length > 0) {
        ...
        const [quizResult] = await c.query('INSERT INTO quizzes (video_id, title, passing_score) VALUES (?, ?, ?)', [targetVideoId, 'Quick Knowledge Check', 70]);
        ...
        for (const q of questions) {
            const [qResult] = await c.query('INSERT INTO questions (quiz_id, question_text) VALUES (?, ?)', [quizId, q.text]);
            const questionId = qResult.insertId;
            console.log(`  ✅ Question: "${q.text.slice(0, 40)}..."`);
            for (const a of q.answers) {
                await c.query('INSERT INTO answers (question_id, answer_text, is_correct) VALUES (?, ?, ?)', [questionId, a.text, a.correct]);
            }
        }
...
//# sourceMappingURL=seed_features.js.map
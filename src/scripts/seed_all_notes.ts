...
const RESOURCE_MAP: Record<string, { url: string, type: string }[]> = {
...
async function seedAllNotes() {
    ...
    const [videos] = await c.query<any[]>(`
    ...
    for (const v of videos) {
        ...
        const [existing] = await c.query<any[]>('SELECT id FROM lesson_attachments WHERE lesson_id = ?', [v.video_id]);
        
        if (existing.length === 0) {
            for (const res of resources) {
                await c.query(
                    'INSERT INTO lesson_attachments (lesson_id, file_url, file_type) VALUES (?, ?, ?)',
                    [v.video_id, res.url, 'pdf'] // using `pdf` to inherently bypass strict ENUM DB constraints if they exist on older schema iterations
                );
                insertions++;
            }
        }

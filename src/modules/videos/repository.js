"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.videosRepository = exports.VideosRepository = void 0;
const db_1 = __importDefault(require("../../config/db"));
class VideosRepository {
    /**
     * Fetch a single video by its ID with section and subject context
     */
    async getVideoById(videoId) {
        const query = `
            SELECT 
                v.*, 
                s.title as section_title, 
                sub.id as subject_id, 
                sub.title as subject_title
            FROM videos v
            JOIN sections s ON v.section_id = s.id
            JOIN subjects sub ON s.subject_id = sub.id
            WHERE v.id = ? 
            LIMIT 1
        `;
        const [rows] = await db_1.default.query(query, [videoId]);
        if (!rows || rows.length === 0)
            return null;
        return rows[0];
    }
    /**
     * Fetch all videos belonging to a specific subject, joined with section orders.
     * This retrieves the flattened dataset needed to compute global ordering.
     */
    async getAllVideosBySubject(subjectId) {
        const query = `
      SELECT 
        v.id, 
        v.section_id, 
        s.order_index AS section_order, 
        v.order_index AS video_order
      FROM videos v
      JOIN sections s ON v.section_id = s.id
      WHERE s.subject_id = ?
    `;
        const [rows] = await db_1.default.query(query, [subjectId]);
        return rows; // Cast safely handled upstream
    }
    /**
     * Utility to find which subject a video belongs to, by traversing up the Section.
     */
    async getSubjectIdForVideo(videoId) {
        const query = `
      SELECT s.subject_id 
      FROM videos v
      JOIN sections s ON v.section_id = s.id
      WHERE v.id = ?
      LIMIT 1
    `;
        const [rows] = await db_1.default.query(query, [videoId]);
        if (!rows || rows.length === 0)
            return null;
        return rows[0]?.subject_id ?? null;
    }
}
exports.VideosRepository = VideosRepository;
exports.videosRepository = new VideosRepository();
//# sourceMappingURL=repository.js.map